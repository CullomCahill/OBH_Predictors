...
## Predictor Variable Selection (Data Description)
...
ggplot(freq_tab_rm, aes(variable, percent, fill = response)) + geom_bar(position = "dodge", stat = "identity")

rci_tab <- freq_tab_rm  %>% filter(variable == "adopted") 
rci_tab$rci <- ifelse(rci_tab$response == 1, 7, 48)
rci_tab$no_rci <- ifelse(rci_tab$response == 0, 37, 8)

ggplot(rci_tab) + 
  geom_bar(aes(variable, percent, color = rci), stat= "identity") +
  geom_bar(aes(variable, percent, fill = response), position = "dodge", stat = "identity")
  



rci2 <- obh %>%
  group_by(rci, adopted) %>%
  summarise(COUNT = n()) %>%
  ungroup() %>%
  mutate(PERCENT = COUNT/sum(COUNT)) 

rci2.1 <- rci2 %>% 
  mutate(adopted1 = recode(adopted, "0" = "not_adopted", "1" = "adopted"))

str(rci2.1)
rci2.1$rci <- as.factor(rci2.1$rci)

colors <- c("grey", "blue3")
ggplot(rci2.1) + geom_bar(aes(adopted1, PERCENT, fill = rci), stat= "identity") +
...
```{r }
# Make similar object but for RCI
rci_list <- lapply(obh[,c(1, 5:13, 18)], tabyl) 

# Bring this list of tables together into a central table
rci_list_tab <- data.table::rbindlist(rci_list, fill = TRUE, idcol=TRUE)

# Tidy up
rci_tab <- freq_tab_rm %>% rename(variable = `.id`, response = `X[[i]]`) %>% 
  mutate(percent = round(percent*100, 2)) %>% 
  select(-valid_percent)


rci_tab <- obh %>% select(gender, adopted, attention_issue, autism, anxiety, conduct_disorder, depression, substance_abuse, trauma_related_issue, rom, transport, rci) 
